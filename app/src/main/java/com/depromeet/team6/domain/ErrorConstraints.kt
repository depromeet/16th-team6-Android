...
object ToastMessage {
    const val API_ERROR_UNKNOWN = "알 수 없는 서버 오류입니다. 잠시 후 다시 시도해주세요."

    const val API_ERROR_NETWORK_FAILURE = "네트워크 오류가 발생했습니다. 잠시 후 다시 시도해주세요."

    const val API_ERROR_LOGIN_TOKEN_EXPIRED = "로그인 정보가 만료되었습니다. 다시 로그인 해주세요."
    const val API_ERROR_DUPLICATED_TOKEN = "이미 존재하는 회원정보 입니다."

    const val API_ERROR_INVALID_LOCATION = "유효 범위를 벗어났습니다. 위치를 다시 설정해주세요"
    const val API_ERROR_INVALID_CURRENT_LOCATION = "유효범위를 벗어났습니다. 현위치를 다시 확인해주세요."
