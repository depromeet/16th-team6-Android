...
@Singleton
class GetIsServiceRegionUseCase @Inject constructor(
    private val locationsRepository: LocationsRepository
) : NetworkRequestUseCase<GetIsServiceRegionUseCase.Params, Boolean>() {

    data class Params(val lat: Double, val lon: Double)

    suspend operator fun invoke(lat: Double, lon: Double): Result<Boolean> =
        invoke(Params(lat, lon))

    override suspend fun apiCall(params: Params): Result<Boolean> {
        return locationsRepository.getIsServiceRegion(lat = params.lat, lon = params.lon)
    }
