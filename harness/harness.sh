...
while true; do
  ...
  case $PHASE in
    "PLANNING"|"REPLANNING")
      local step_count=$(jq '.steps | length' "$STATE_FILE" 2>/dev/null)
    ...
    "REVIEW_REQUIRED")
      ...
      PROMPT="You are a Senior Architect. Review the following changes based on the goal: $(jq -r '.goal' "$STATE_FILE")\n\n[DIFF SUMMARY]\n$DIFF_STAT\n\n[FULL DIFF]\n$DIFF_FULL\n\nIf OK, set phase to 'COMPLETED'. If not, set to 'REPLANNING' with feedback. Output ONLY valid JSON."
